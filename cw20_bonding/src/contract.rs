...
use crate::msg::{CurveFn, CurveInfoResponse, ExecuteMsg, InstantiateMsg, QueryMsg, CurveType};
...
pub fn do_execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetCurve { curve } => {
            Ok(execute_set_curve(deps, info, curve)?)
        }
    }
}
...
pub fn execute_set_curve(
    deps: DepsMut,
    info: MessageInfo,
    curve: CurveType,
) -> Result<Response, ContractError> {
    check_owner(&deps, &info)?;
    CURVE_TYPE.save(deps.storage, &curve)?;

    // bond them to the validator
    let res = Response::new()
        .add_attribute("action", "set curve type")
